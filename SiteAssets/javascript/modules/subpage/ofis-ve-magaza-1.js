document.addEventListener("DOMContentLoaded", function () {
      ...
      .then((data) => {
        ...
        partnersAll.forEach(function (location) {
          ...
          marker.on("click", function () {
            mymap.flyTo([location.CoordinateX + 0.0005, location.CoordinateY], 18, {
              animate: true,
              duration: 3,
            });
          });
  ...
  document
    ...
    .addEventListener("input", function (e) {
      ...
      var filteredCounties = counties.filter((district) => district.toUpperCase().includes(e.currentTarget.value.toUpperCase()));

      filteredCounties.forEach((district) => {
        districtList.insertAdjacentHTML("beforeend", generateCityAndDistrictButtons(null, district));
      });

      // Arama sonrası ilçe seçim işlemlerini yeniden bağla
      setDistrict();
    });
    ...
    mutationsList.forEach((mutation) => {
      if (mutation.type === "attributes" && mutation.attributeName === "class") {
        ...
        if (target.classList.contains("show")) {
          ...
          observer.disconnect(); // İlk müdahaleden sonra gözlemciyi devre dışı bırak
        }
      }
    });
  });

  const dropdownMenus = document.querySelectorAll(".searchCardAndMapContainer .dropdown-menu");
  ...
  function showResult() {
      ...
      return `
          <div class="d-flex gap-3 resultCard">
            <div class="d-flex flex-column popupTextContainer">
              <span class="fw-bold">${partner.CommercialName}</span>
              <span>${partner.Street}</span>
              <span>No:${partner.BuildingNo}${partner.HouseNo != "UNKNOWN" && partner.HouseNo != "X" ? "/" + partner.HouseNo : ""} ${partner.PostalCode}</span>
              <span>${partner.District}/${partner.City}</span>
              <span>İletişim Bilgisi: <a href="tel:0${partner.ContactTel}">${partner.ContactTel}</a></span>
            </div>
            <div class="popupButtonContainer">
              <button type="button" class="popupButton" onclick="shareClick(event)" data-bs-toggle="modal" data-bs-target="#shareModal" data-lat="${
                partner.CoordinateX
              }" data-long="${partner.CoordinateY}"><img src="https://asset.turktelekom.com.tr/SiteAssets/images/new_map/share_icon.webp" width="24" height="25"></button>
              <button type="button" class="popupButton" onclick="addressClick(event)" data-lat="${
                partner.CoordinateX
              }" data-long="${partner.CoordinateY}"><img src="https://asset.turktelekom.com.tr/SiteAssets/images/new_map/address_icon.webp" width="24" height="25"></button>
            </div>
          </div>
        `;
    }
    console.log("Sonuçları göster");
    ...
    const searchboxValue = replaceText(searchInput.value);
    const selectedCityValue = replaceText(selectedCity.innerHTML);
    const selectedDistrictValue = replaceText(selectedDistrict.innerHTML);
    ...
    // Eğer üçü de boşsa, tüm sonuçları göster
    if (!searchboxValue && selectedCityValue === "lutfen il seciniz" && selectedDistrictValue === "lutfen ilce seciniz") {
      partnersAll.forEach((partner) => {
        searchResultContainer.insertAdjacentHTML("beforeend", addResult(partner));
      });
      return;
    }

    const filteredPartners = partnersAll.filter((partner) => {
      const partnerName = replaceText(partner.CommercialName);
      const partnerCity = replaceText(partner.City);
      const partnerDistrict = replaceText(partner.District);

      const matchesSearchbox = searchboxValue ? partnerName.includes(searchboxValue) : true;
      const matchesCity = selectedCityValue !== "lutfen il seciniz" ? partnerCity === selectedCityValue : true;
      const matchesDistrict = selectedDistrictValue !== "lutfen ilce seciniz" ? partnerDistrict === selectedDistrictValue : true;

      return matchesSearchbox && matchesCity && matchesDistrict;
    });
  ...
  // "Sonuçları Göster" butonuna tıklama olayını dinle
